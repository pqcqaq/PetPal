...
- PetPal 治理工作区：根级 `/petpal-admin` 已覆盖总览、投诉、违规处罚、整改附件直传与复核、处罚模板、处罚申诉审核、照料者审核、回调治理与平台规则管理，附件中心也已补上资质/订单消息/投诉/整改等 PetPal 业务引用展示、删除保护和临时业务附件回收，订单消息/投诉附件上传也已开始按订单范围治理，当前 Web 订单详情与跨订单消息中心、Uni 订单详情都已能直接发送图片消息附件。
- Uni 移动端：自定义 Header、Tabbar、安全区适配，不依赖第三方 UI 组件库，照料者资质材料、投诉证据和订单消息图片上传已对齐后台附件治理标签，当前订单详情与跨订单消息中心都可直接发送图片消息附件。
