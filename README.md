...
- PetPal 治理工作区：根级 `/petpal-admin` 已覆盖总览、投诉、违规处罚、整改附件直传与复核、处罚模板、处罚申诉审核、照料者审核、回调治理与平台规则管理，附件中心也已补上资质/订单消息/投诉/整改等 PetPal 业务引用展示、删除保护和临时业务附件回收，订单消息/投诉附件上传也已开始按订单范围治理，当前 Web / Uni 订单详情都已能直接发送图片消息附件。
