...
- 新增独立业务后台命名空间（例如 `/petpal-admin/*`）时，必须同时同步：
  - 后端入口权限
  - 前端直达导航
  - 路由注册
  - 默认落点判断
- 当某个后台页面允许由多个同级权限之一进入时，前后端必须同步使用“任一权限可进入”的约定：
  - 后端路由使用 `requireAnyPermission(...)`
  - 前端路由 meta 使用 `permissionAny`
  - 导航可见性与默认落点判断同步按同一组权限放行
