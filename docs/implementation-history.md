...
- `/petpal/messages` 为新的跨订单消息中心，并已接入未读筛选、按角色聚合、线程详情直读、快捷回复、订单范围图片上传、按线程保留草稿和订单快捷跳转。
...
- App 订单详情页与跨订单消息中心的消息图片上传已切到订单范围治理标签，发送前支持上传、预览和移除，发出后会直接进入订单消息会话，不再要求手填外链 URL；消息中心当前也已补齐主人 / 照料者双视角、线程详情直读、就地已读、快捷回复和按线程保留草稿。
