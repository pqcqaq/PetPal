...
- 2026-04-04 已把 `apps/app-frontend` 的 Vite 解析改为对 `@rbac/api-common` 直连工作区源码，并显式排除依赖预构建，修复 `owner-shared.ts` 新增 `formatPetPalAmount` 等导出后仍被旧 `.vite/deps/@rbac_api-common.js` 命中的白屏问题。
...
- 2026-04-04 已继续收口消息草稿持久化快照 helper：
...
- 2026-04-04 已继续收口消息草稿 mutation helper：
...
- 2026-04-04 已继续收口 PetPal 会话展示 helper：
  - `packages/api-common/src/helpers/petpal-display.ts` 已新增 `formatPetPalConversationPreview(...)` 与 `formatPetPalConversationMeta(...)`，统一处理最近消息回退文案、会话时间和未读状态拼接。
  - Web `shared.ts` 与 App `owner-shared.ts` 现在都已直接复用这组共享 helper，不再各自维护第二份近似的 preview / hint 逻辑；双端只继续保留各自的 copy 和时间格式化器。
  - `apps/web-frontend/test/petpal-shared.test.ts` 已补可配置 preview/meta helper 的直测，继续兜底共享展示逻辑不会漂移。
- 2026-04-04 已继续收口 PetPal 状态标签 helper：
  - `packages/api-common/src/helpers/petpal-display.ts` 已新增订单状态、需求状态、退款进度以及投诉类型/对象/状态的共享 label/hint helper。
  - Web `shared.ts` 与 App `owner-shared.ts` 现在都已直接复用这组共享 helper，不再各自维护第二份近似的状态文案映射；App `rebuild/shared.ts` 也已改为直接消费 helper，而不是回读一组本地 label record。
  - `apps/web-frontend/test/petpal-shared.test.ts` 已补这组状态展示 helper 的稳定性断言，继续兜底共享口径不会漂移。
- 2026-04-04 已继续收口 PetPal 订单过滤 predicate：
  - `packages/api-common/src/helpers/petpal-display.ts` 已新增订单待支付、售后跟踪和主人订单过滤的共享 predicate/helper。
...
- 2026-04-04 已继续收口 PetPal 服务记录标签与订单 tone helper：
  - `packages/api-common/src/helpers/petpal-display.ts` 已新增 `ServiceLogType` 标签 helper 和订单 tone helper。
  - Web `shared.ts` 与 App `owner-shared.ts` 现在都已直接复用这组共享 helper，不再各自维护第二份服务记录标签和订单 tone 映射；App `rebuild/shared.ts` 里的 `serviceLogOptions` 也已改为直接消费共享标签 helper。
  - `apps/web-frontend/test/petpal-shared.test.ts` 已补这组 helper 的稳定性断言，继续兜底共享展示口径不会漂移。
- 2026-04-04 已继续收口 PetPal 服务类型标签 helper：
  - `packages/api-common/src/helpers/petpal-display.ts` 已新增 `PetServiceType` 标签 helper。
  - Web `shared.ts` 与 App `owner-shared.ts` 现在都已直接复用这组共享 helper，不再各自维护第二份服务类型中文映射；Web / App 两端的服务类型选项和 App 的 `serviceTypeLabels` 也已同步切到共享标签来源。
  - `apps/web-frontend/test/petpal-shared.test.ts` 已补这组 helper 的稳定性断言，继续兜底共享展示口径不会漂移。
- 2026-04-04 已继续收口 PetPal 支付渠道标签 helper：
  - `packages/api-common/src/helpers/petpal-display.ts` 已新增 `OwnerPayChannel` 标签 helper。
  - Web `shared.ts` 与 App `owner-shared.ts` / `rebuild/shared.ts` 现在都已直接复用这组共享 helper，不再各自维护第二份支付渠道中文映射；Web / App 两端的支付渠道选项当前都已同步切到共享标签来源。
  - `apps/web-frontend/test/petpal-shared.test.ts` 已补这组 helper 的稳定性断言，继续兜底共享展示口径不会漂移。
...
- 2026-04-04 已继续收口 App 主人端交易后段焦点回流：
  - `apps/app-frontend/src/pages/petpal/orders.vue` 的“处理售后”动作已开始带 `focusOrderId + filter` 打开售后中心，不再只做无上下文跳转。
...
- 2026-04-04 已继续收口 App 订单详情来源说明：
  - `apps/app-frontend/src/pages/petpal/owner-shared.ts` 已新增 `order-detail` page context，支持暂存 `orderId / tab / source`。
...
- App 端订单详情已经基本完成“首屏决策 + 分栏任务区”重构，订单列表、售后中心和四类结果页之间也已补第一轮焦点回流，但订单详情、消息中心与更细的 page notice / 主动提醒仍可继续增强。
- App 端订单列表已经开始从“筛选 + 卡片列表”转向“任务队列”，结果页回流也已开始携带筛选和焦点订单，但与消息中心、提醒中心之间的主动分发仍可继续增强。
