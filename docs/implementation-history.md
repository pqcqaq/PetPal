...
- Web 前台已开始按角色拆路由：
  ...
  - `/petpal/caregiver/earnings` 为新的照料者收益页，并已切到后端收益摘要接口，统一展示累计收入、服务结构、售后风险、最近售后风险订单和日 / 周 / 月趋势，同时已补经营明细导出入口、导出筛选条、快捷时间窗、最近一次筛选持久化、常用模板、订单号关键词专项导出、退款类型专项导出、退款状态专项导出、退款原因关键词专项导出、退款风险单专项导出、投诉状态专项导出、投诉类型专项导出、投诉摘要关键词专项导出和投诉责任角色专项导出；当前命名导出模板的校验、upsert、删除规则、模板选中态管理、模板动作区 UI、导出工具条壳层、字段绑定规则、日期范围桥接、收益导出快照状态、snapshot clone/apply 字段拷贝、query 值归一化、has-filters 判定以及 defaults/keys 定义都已开始共享化，收益页当前已把售后风险单升级成带投诉/退款原因的复盘队列，并支持从单笔风险单一键切到“同类风险”导出条件，也支持按待受理投诉、平台责任和待退款直接切到队列级导出条件，还会高亮当前已生效的风险导出视角并支持回到全部风险队列，在没有履约订单时也会优先指向最需要复盘的售后对象。
