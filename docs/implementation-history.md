...
## 88. App 端“我的”页改为 PetPal 账户能力视图（P1-M3 Slice 61）

**内容**：继续清理移动端剩余模板语义，本轮重构 `app-frontend` 的“我的”页，把“角色数量 / 当前角色 / 权限数量”改成 PetPal 账户定位、工作区和能力表达。

变更摘要：

- `apps/app-frontend/src/pages/me/me.vue`
  - 新增基于现有 `roles` 与 `permissions` 的 PetPal 账户层级推导：
    - `平台全量治理账号`
    - `运营协同账号`
    - `主人服务账号`
  - 头像区标签从原始角色名切换为 PetPal 账户定位，不再直接向移动端显示 RBAC 角色概念。
  - “账号状态”区重构为“账户能力”：
    - 账号状态
    - 账号定位
    - 可用工作区
    ...
    - 当前体验设置
  - 页面继续复用现有真实能力判断：
    - 后台治理入口根据 `petpal.complaint.* / petpal.caregiver.* / petpal.callback-*` 权限前缀推导
    - 资料上传根据 `file.upload.avatar / file.upload` 推导
  - 本轮不改动接口、路由、鉴权或持久化，仅做表达层去模板化。
...
- 已确认“我的”页不再展示角色数量和权限数量等底层模板指标，但底层能力判断仍沿用现有权限数据源，没有放宽访问控制。
- 已确认后台工作区展示改为能力提示，不会绕过实际路由守卫和后端权限校验。
- 已确认本轮只影响移动端页面文案和前端派生字段，不影响数据加载与订单/宠物/需求统计逻辑。

Git commit：待本切片提交。

## 89. Web 公开辅助页收口为 PetPal 入口语义（P1-M3 Slice 62）

**内容**：继续清理 Web 公开面的模板式措辞，本轮把 404、OAuth 授权页和主人端媒体上传提示统一改为 PetPal 产品语言，并收口首页结构文案中的内部实现表述。

变更摘要：

- `apps/web-frontend/src/pages/frontend/not-found/NotFoundView.vue`
  - 404 页提示不再引导“进入控制台”，改为回到主人服务台与 PetPal 后台。
  - 未登录用户主按钮改为进入 `/petpal` 主人服务台；已登录用户仍可直接返回 `/petpal-admin`。
- `apps/web-frontend/src/pages/frontend/oauth/OAuthAuthorizeView.vue`
  - 加载态和授权说明从“权限范围”改为“本次授权内容”。
  - 第三方应用描述收口为 PetPal 账号资料与接入授权内容，不再用模板式权限表达。
- `apps/web-frontend/src/pages/frontend/oauth/OAuthAuthorizeErrorView.vue`
  - 错误页按钮改为按登录态跳转：
    - 已登录回主人服务台
    - 未登录前往登录
  - 错误提示改为围绕“业务应用重新发起接入授权”描述。
- `apps/web-frontend/src/pages/frontend/frontend-content.ts`
  - 公开页能力卡与架构说明中的“按权限显示 / 权限校验 / RBAC 介绍”等文案改成“按账号已开通能力 / 访问控制 / 通用框架介绍”等更产品化表达。
- `apps/web-frontend/src/pages/frontend/architecture/ArchitectureView.vue`
  - 结构页首屏改为强调后台入口已经改成根级直达。
- `apps/web-frontend/src/pages/frontend/petpal/PetPalOwnerView.vue`
  - 服务记录媒体上传提示不再直接暴露 `file.upload` 权限码。
  - 失败提示改为“媒体上传能力未开通”的产品语言。
...
- 已确认 404 与 OAuth 页只调整公开文案和跳转目标，没有改动 OAuth 会话 API、鉴权流程或路由守卫。
- 已确认主人端服务记录上传仍使用原有真实能力判断，只是去掉了面对用户的权限码暴露。
- 已确认公开首页结构说明仍保留访问控制事实，但不再以模板实现概念作为对外主叙事。

Git commit：待本切片提交。

## 90. PetPal 根级后台首页升级为治理摘要页（P1-M3 Slice 63）

**内容**：继续响应“PetPal 后台应直接定义在根下、方便快捷使用”的要求，本轮把 `/petpal-admin` 首页从单纯入口聚合页升级为可刷新的治理摘要页，直接聚合现有投诉、照料者审核、回调审计与告警队列统计接口。

变更摘要：

- `apps/web-frontend/src/pages/petpal-admin/PetPalAdminHubView.vue`
  - 新增治理摘要加载流程，按当前账号实际权限动态拉取：
    - 投诉统计 `complaintStats`
    - 待审核照料者数量（复用 `caregiverAudits` 分页总数）
    - 回调审计统计 `callbackAuditStats`
    - 告警队列统计 `callbackAlertOutboxStats`
  - 首页从“仅显示可进入工作区数量”升级为三层信息结构：
    - 顶部信号卡：可访问工作区、当前账号、最近刷新时间
    - 核心指标卡：超时投诉、待审照料者、回调成功率、死信告警
    - 优先关注区：基于超时投诉、待审审核、死信/卡住告警生成快速处理入口
  - 新增“刷新治理摘要”按钮，支持在首页快速重拉当前账号可见统计。
  - 对部分统计加载失败场景增加非阻断提示，不影响继续进入具体工作区。
...
- 已确认本轮只复用现有后台统计接口，没有新增后端接口或放宽权限边界。
- 已确认各指标仅在当前账号具备对应读取权限时才请求，避免首页盲目触发无权接口。
- 已确认统计加载失败时仍保留已有后台入口卡片，不会阻断投诉、审核、回调治理的直接进入。
...
## 92. PetPal 根级后台前移值班动作（P1-M3 Slice 65）

**内容**：继续把根级后台从“摘要页”推进成可直接值班的工作台，本轮在 `/petpal-admin` 首页前移高频值班动作，让当前账号可以从首页直接跳我的工单、待审审核，并在有权限时直接重试死信告警。
...
- `apps/web-frontend/src/pages/petpal-admin/PetPalAdminHubView.vue`
  - 新增“值班动作”区块：
    ...
    - 处理待审照料者
    - 重试死信告警
  - “查看我的工单”直接带 `assignedAdminId` 查询参数进入投诉工单页。
  - “处理待审照料者”直接带 `auditStatus=PENDING` 进入审核页。
  - “重试死信告警”复用现有 `retryDeadCallbackAlertOutbox(50)` 接口，在首页直接触发兜底重试，并在完成后刷新治理摘要。
  - 按当前账号实际权限控制值班动作是否显示，避免暴露无权操作入口。
...
- 已确认首页前移的死信重试仍复用原有后台权限点 `petpal.callback-alert.retry`，没有新增绕过治理页的快捷后门。
- 已确认“我的工单 / 待审审核”仅是预设过滤直达，不改变投诉和审核页本身的访问控制与列表来源。
- 已确认死信重试成功后会重新拉取根级治理摘要，避免首页指标停留在旧状态。
...
## 96. PetPal 根级后台接管告警队列页面实现（P1-M3 Slice 69）

**内容**：继续把 PetPal 后台治理页从旧 `console/petpal` 目录平移到根级命名空间，本轮将回调告警队列页的真实实现迁入 `pages/petpal-admin/callback-alert-outbox`，让 `/petpal-admin/callback-alert-outbox` 不再依赖旧模板页面文件。

变更摘要：

- `apps/web-frontend/src/pages/petpal-admin/callback-alert-outbox/PetPalCallbackAlertOutboxAdminView.vue`
  - 新增根级后台告警队列主页面，实现队列筛选、死信重试、重放记录抽屉和统计摘要。
  - 保留现有 URL 状态恢复、重放记录导出与死信批量重试逻辑，不改变后端接口契约。
- `apps/web-frontend/src/pages/petpal-admin/PetPalCallbackAlertOutboxRouteView.vue`
  - 根级路由包装层改为直接引用新的 `petpal-admin` 页面实现。
- `apps/web-frontend/src/pages/console/petpal/CallbackAlertOutboxView.vue`
  - 旧页面退化为兼容壳层，仅转发到新的根级后台页面实现。
...
- 已确认 `/petpal-admin/callback-alert-outbox` 路由不再依赖旧 `console/petpal` 页面文件，根级后台开始直接持有告警队列实现。
- 已确认旧入口仍保留兼容壳层，迁移期间内部残留跳转不会失效。
- 已确认本轮未改动任何回调告警后端接口、权限点或筛选参数，仅做前端实现位置迁移，回归风险较低。
...
## 98. PetPal 根级后台接管投诉工单页面实现（P1-M3 Slice 71）

**内容**：继续把 PetPal 后台治理页从旧模板目录完全迁出，本轮将投诉工单页的真实实现迁入 `pages/petpal-admin/complaints`，让 `/petpal-admin/complaints` 不再依赖旧 `console/petpal` 页面文件。

变更摘要：

- `apps/web-frontend/src/pages/petpal-admin/complaints/PetPalComplaintAdminView.vue`
  - 将投诉工单页真实实现平移至根级后台命名空间。
  - 保留现有批量分配、批量结案、快捷接手、SLA 展示、筛选恢复与处理弹窗逻辑。
  - 页面 `viewKey` 同步调整为根级后台语义，减少旧模板痕迹。
- `apps/web-frontend/src/pages/petpal-admin/PetPalComplaintAdminRouteView.vue`
  - 根级路由包装层改为直接引用新的 `petpal-admin` 页面实现。
- `apps/web-frontend/src/pages/console/petpal/ComplaintAdminView.vue`
  - 旧页面退化为兼容壳层，仅转发到新的根级后台页面实现。
...
- 已确认 `/petpal-admin/complaints` 路由不再依赖旧 `console/petpal` 页面文件，根级后台已直接持有投诉工单实现。
- 已确认投诉工单页的筛选、批量动作、快捷接手、SLA 展示和处理弹窗逻辑均保持原有行为，没有新增接口分支。
- 已确认旧入口仍保留兼容壳层，迁移期间不会影响历史入口或内部残留导航。

Git commit：待本切片提交。

## 99. PetPal 根级后台补齐默认落点与实时同步适配（P1-M3 Slice 72）

**内容**：主要治理页已经迁入根级后台后，本轮继续收口入口与运行时行为，补齐登录后的默认后台落点、根级后台的实时同步适配，并清理公开前台与后台壳层里残留的 `console` / `RBAC Control` 语义。

变更摘要：

- `apps/web-frontend/src/realtime/admin-sync.ts`
  - 实时同步路由校正逻辑从仅覆盖 `/console/**` 扩展到同时覆盖 `/petpal-admin/**`。
  - 当根级后台页面权限被回收时，会回退到 `/petpal-admin`，不再只针对旧控制台路径处理。
- `apps/web-frontend/src/pages/console/auth/LoginView.vue`
  - 登录、注册与 OAuth 登录完成后，默认优先落到 `/petpal-admin`（当当前账号具备 PetPal 后台权限时）。
  - 第三方登录默认回跳目标同步改为根级后台优先。
  - 登录页能力说明改为 PetPal 产品语义，不再强调“同一控制台管理”。
...
- `apps/web-frontend/src/pages/frontend/frontend-content.ts`
  - 公开前台组件中的 `consoleTarget` / `consoleLabel`、`consoleHighlights`、`HomeConsolePreview` 等命名统一切为 PetPal Admin 语义。
  - 公开页继续引导进入根级 `/petpal-admin`，但组件与样式命名不再沿用旧 `console` 概念。
- `apps/web-frontend/src/layouts/ConsoleLayout.vue`
  - 后台壳层中的用户可见品牌从 `RBAC Control` / `RBAC Admin` 调整为 `PetPal Admin` / `宠托帮后台`。
...
- 已确认根级 `/petpal-admin/**` 页面现在也会纳入后台实时同步后的权限与路由校正逻辑，不再只覆盖旧 `/console/**`。
- 已确认登录与第三方登录在具备 PetPal 后台权限时会优先进入根级后台，符合“直接定义在根下”的要求。
- 已确认公开前台和后台壳层中残留的 `console` / `RBAC Control` 用户可见语义已进一步收敛，没有改变既有接口契约。
...
## 86. Web 公开前台改为 PetPal 产品文案（P1-M3 Slice 59）

**内容**：继续响应“项目模板要完全改成 PetPal 落地实现”的方向，本轮把 `web-frontend` 公开前台中残留的“权限控制台 / RBAC 示例”文案整体替换为 PetPal 产品表达。
