...
- 2026-04-03 已补照料者收益摘要接口、经营明细导出和导出筛选，后端现在可直接输出累计收入、近 30 天收入、售后风险、最近完成订单、最近售后风险订单、服务收入结构、日 / 周 / 月趋势，以及只包含当前照料者已完成订单的经营 Excel 明细，并支持按时间范围、服务类型、订单号关键词、最低退款金额、最少投诉数、退款类型、退款状态、退款原因关键词、退款风险视角、投诉状态、投诉类型、投诉摘要关键词和投诉责任角色导出；Web 收益页现已继续补上快捷时间窗、最近一次导出条件持久化、常用导出模板、订单号关键词专项导出、退款类型专项导出、退款状态专项导出、退款原因关键词专项导出、退款风险单专项导出、投诉状态专项导出、投诉类型专项导出、投诉摘要关键词专项导出和投诉责任角色专项导出，并已开始把命名导出模板的校验、upsert、删除规则抽到共享 helper、把模板选中态和增删改应用动作抽到共享 composable、把模板选择与应用/保存/删除操作区抽到共享组件、把导出工具条容器与筛选区壳层抽到共享组件、把 clearable/trimmed/page-state 字段绑定规则抽到共享 helper、把导出日期范围解析与写回逻辑抽到共享 helper、把照料者收益导出的快照 clone / apply / query 规则抽到独立状态模块，并把三份导出状态模块重复的 snapshot clone/apply 字段拷贝、query 值归一化规则、has-filters 存在性判断以及 defaults/keys 定义再收口到共享 helper；主人端售后中心现已补上退款明细导出筛选、最近一次筛选持久化和常用模板，主人订单页也已补上交易记录导出筛选、最近一次筛选持久化和常用模板，三张导出页现还会直接展示当前已生效的导出条件并支持逐项移除，收益页也会直接列出带投诉/退款原因的售后风险队列并可一键切到仅导出风险单，风险队列现按争议状态、投诉阶段和退款信号排序，并支持从单笔风险单一键生成“同类风险”导出条件，也支持按待受理投诉、待受理重复投诉、处理中投诉、重复投诉、照料者责任、照料者重复投诉、平台责任、待退款和高退款暴露直接切到队列级导出视角，还会高亮当前已生效的风险导出视角并支持一键恢复到全部风险队列；当前风险预设标签与队列计数规则也已抽到共享 helper，并补了独立单测兜底，Web / App 不再各自二次聚合订单。
