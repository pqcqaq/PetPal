...
- 2026-04-03 已补照料者收益摘要接口、经营明细导出和导出筛选，后端现在可直接输出累计收入、近 30 天收入、售后风险、最近完成订单、服务收入结构、日 / 周 / 月趋势，以及只包含当前照料者已完成订单的经营 Excel 明细，并支持按时间范围、服务类型、订单号关键词、退款类型、退款状态、退款原因关键词、退款风险视角、投诉状态、投诉类型、投诉摘要关键词和投诉责任角色导出；Web 收益页现已继续补上快捷时间窗、最近一次导出条件持久化、常用导出模板、订单号关键词专项导出、退款类型专项导出、退款状态专项导出、退款原因关键词专项导出、退款风险单专项导出、投诉状态专项导出、投诉类型专项导出、投诉摘要关键词专项导出和投诉责任角色专项导出，并已开始把命名导出模板的校验、upsert、删除规则抽到共享 helper、把模板选中态和增删改应用动作抽到共享 composable、把模板选择与应用/保存/删除操作区抽到共享组件、把导出工具条容器与筛选区壳层抽到共享组件、把 clearable/trimmed/page-state 字段绑定规则抽到共享 helper、把导出日期范围解析与写回逻辑抽到共享 helper，并把照料者收益导出的快照 clone / apply / query 规则抽到独立状态模块；主人端售后中心现已补上退款明细导出筛选、最近一次筛选持久化和常用模板，主人订单页也已补上交易记录导出筛选、最近一次筛选持久化和常用模板，Web / App 不再各自二次聚合订单。

### 2.2 Web 端

- 主人工作台已具备宠物建档、需求发布、订单跟踪、退款/投诉进度、交易与退款导出等能力。
- Web 前台已开始按角色拆路由：
  - `/petpal` 为新的主人服务台，并已接入交易导出筛选、交易导出模板、退款导出、筛选模板和确认完成动作；主人订单页与售后中心现已复用同一套导出模板动作组件、导出工具条壳层、字段绑定 helper 和日期范围 helper。
  - `/petpal/caregiver` 为新的照料者工作台，并已接入资质材料上传、服务记录和签退动作。
  - `/petpal/caregiver/earnings` 为新的照料者收益页，并已切到后端收益摘要接口，统一展示累计收入、服务结构、售后风险和日 / 周 / 月趋势，同时已补经营明细导出入口、导出筛选条、快捷时间窗、最近一次筛选持久化、常用模板、订单号关键词专项导出、退款类型专项导出、退款状态专项导出、退款原因关键词专项导出、退款风险单专项导出、投诉状态专项导出、投诉类型专项导出、投诉摘要关键词专项导出和投诉责任角色专项导出；当前命名导出模板的校验、upsert、删除规则、模板选中态管理、模板动作区 UI、导出工具条壳层、字段绑定规则、日期范围桥接以及收益导出快照状态都已开始共享化。
  - `/petpal/reminders` 为新的提醒中心，并已接入高优先待办、近 48 小时安排和主人/照料者双视角分发。
  - `/petpal/messages` 为新的跨订单消息中心，并已接入未读筛选、按角色聚合和订单快捷跳转。
  - `/petpal/aftersales` 为新的主人售后中心，并已接入退款进度、投诉状态、优先级排序、退款明细导出筛选、最近一次退款导出条件持久化和退款导出模板，并已与主人订单页、照料者收益页复用同一套模板动作壳层、导出工具条容器、字段绑定 helper 和日期范围 helper。
