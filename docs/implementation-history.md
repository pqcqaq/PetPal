...
- Web 前台已开始按角色拆路由：
  - `/petpal` 为新的主人服务台，并已接入交易导出筛选、交易导出模板、退款导出、筛选模板和确认完成动作；主人订单页与售后中心现已复用同一套导出模板动作组件、导出工具条壳层和字段绑定 helper。
  - `/petpal/caregiver` 为新的照料者工作台，并已接入资质材料上传、服务记录和签退动作。
  - `/petpal/caregiver/earnings` 为新的照料者收益页，并已切到后端收益摘要接口，统一展示累计收入、服务结构、售后风险和日 / 周 / 月趋势，同时已补经营明细导出入口、导出筛选条、快捷时间窗、最近一次筛选持久化、常用模板、订单号关键词专项导出、退款类型专项导出、退款状态专项导出、退款原因关键词专项导出、退款风险单专项导出、投诉状态专项导出、投诉类型专项导出、投诉摘要关键词专项导出和投诉责任角色专项导出；当前命名导出模板的校验、upsert、删除规则、模板选中态管理、模板动作区 UI、导出工具条壳层以及字段绑定规则都已开始共享化。
  - `/petpal/reminders` 为新的提醒中心，并已接入高优先待办、近 48 小时安排和主人/照料者双视角分发。
  - `/petpal/messages` 为新的跨订单消息中心，并已接入未读筛选、按角色聚合和订单快捷跳转。
  - `/petpal/aftersales` 为新的主人售后中心，并已接入退款进度、投诉状态、优先级排序、退款明细导出筛选、最近一次退款导出条件持久化和退款导出模板，并已与主人订单页、照料者收益页复用同一套模板动作壳层、导出工具条容器和字段绑定 helper。
