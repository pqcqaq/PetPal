...
- Web 前台已开始按角色拆路由：
  - `/petpal` 为新的主人服务台。
  - `/petpal/caregiver` 为新的照料者工作台。
  - `/petpal/legacy` 保留为兼容混合工作台。
...
- 第十四阶段开始推进 Web 前台真实路由拆分，把主人主流程和照料者主流程从旧混合工作台中切出，并保留 `legacy` 作为兼容过渡层。

## 4. 下一步建议

- 第一优先级继续是 App / Web 用户操作逻辑页面重构，当前 Web 端要先继续压缩 `legacy`，把照料者资质上传、服务记录媒体上传和签退等高频动作迁到独立照料者页。
