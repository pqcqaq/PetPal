...
- PetPal 后台治理接口，已覆盖总览、投诉、违规处罚 / 整改附件直传与复核 / 处罚模板 / 单次申诉审核、照料者审核、回调审计 / 告警与平台规则草稿 / 发布 / 归档；附件中心也已能识别照料者资质材料、订单投诉证据与处罚整改材料这三类 PetPal 业务引用，并在删除前做业务级拦截，同时会定时回收长期未被业务引用的临时资质/投诉/整改附件，订单参与方现在也可按订单范围上传消息/投诉证据附件而不依赖通用 `file.upload`
