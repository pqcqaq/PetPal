...
- PetPal Web `shared.ts` 与 App `owner-shared.ts` / `rebuild/shared.ts` 里的支付渠道标签 helper 当前也已继续收口到 `@rbac/api-common`；Web / App 两端的支付渠道选项当前都已开始复用同一套 `OwnerPayChannel -> 中文标签` 映射，只继续保留各自的 note 文案差异。
...
- PetPal App 订单列表、售后中心与支付 / 退款 / 投诉 / 评价结果页当前已接入 `focusOrderId + filter` 页面上下文；用户完成支付、退款、投诉或评价后，会回到正确队列并把当前订单顶到前面，`apps/web-frontend/test/petpal-shared.test.ts` 也已补对应回归断言，避免后续再次退化成裸跳转
- PetPal App 消息中心、提醒中心与通知动作当前也已接入同一套页面上下文与上下文感知跳转；通知、起步向导、首页催办信号和角色导航进入消息页时会带 `role + filter + focusOrderId`，进入提醒中心时会带 `scope + focusNotificationId`，消息页会优先承接当前焦点订单，提醒中心也会按范围筛选并把当前待办顶到前面，避免用户从通知或结果页回流后再次手动找线程或提醒
- PetPal App 订单详情当前也已接入轻量 page-context 来源说明；从消息中心和支付 / 退款 / 投诉 / 评价结果页进入订单详情时，会暂存 `orderId + tab + source`，详情页首屏会解释“为什么把你带到这里”，不再只做无说明的分栏落页
- PetPal App 订单详情来源说明当前也已扩到高频摘要与队列入口；主人首页、订单队列、售后中心、照料者首页、履约订单和收益页进入订单详情时，也会带上 `source`，详情页不再只区分消息或结果页回流
- PetPal App 通知 / 提醒到订单详情的高频直达当前也已开始收口；通知中心与提醒中心打开单笔订单类待办时，会优先直达订单详情的沟通 / 履约 / 售后分栏，并带上 `notifications / reminders` 来源说明；多订单聚合项仍保留消息中心、售后中心或履约队列作为中间分发层
- PetPal App 订单详情当前也已开始承接更细的 entry reason；结果页和通知直达详情时，会额外写入 `reason`，详情页首屏会进一步解释“现在先做什么”，而不只停留在来源说明
- PetPal App 订单 / 售后 / 消息中心当前也已开始透传 `detailReason`；结果页或通知先落到中间页后，再进入订单详情时，仍能保留上一跳的动作原因，不会退化成普通列表入口
