...
<template>
  <PetpalPage
    title="主人首页"
    :subtitle="tokenStore.hasLogin ? '先选本次要操作的宠物，再继续需求、订单或沟通。' : '登录后才能查看宠物、需求和订单。'"
    eyebrow="Owner"
    :with-tabbar="true"
  >
    <template #bar>
    ...
    <template v-else>
      <PetpalSection tone="accent">
        <view class="petpal-inline">
          <view class="petpal-inline petpal-inline--start">
            ...
            <view class="petpal-stack petpal-stack--tight">
              ...
              <text class="petpal-note">先明确这次照料的是哪只宠物，再继续需求、订单和沟通。</text>
            </view>
          </view>
        ...
        <view class="petpal-action-row">
          <button class="petpal-btn petpal-btn--primary" hover-class="none" @click="openCreateRequest(selectedPet?.id)">为当前宠物新建需求</button>
          <button class="petpal-btn petpal-btn--secondary" hover-class="none" @click="openPets">管理宠物档案</button>
        </view>
      </PetpalSection>

      <PetpalSection title="当前操作对象" subtitle="一次只处理一只宠物，避免切换时把需求和订单混在一起。">
        <template v-if="pets.length">
          <view class="petpal-choice-grid petpal-choice-grid--two">
            <button
              v-for="pet in pets"
              :key="pet.id"
              :class="['petpal-choice-tile', selectedPetId === pet.id ? 'petpal-choice-tile--active' : '']"
              hover-class="none"
            ...
            >
              ...
              <text class="petpal-choice-tile__meta">{{ describePet(pet) }}</text>
              <text class="petpal-choice-tile__hint">{{ selectedPetId === pet.id ? '当前正在处理' : '切换到这只宠物' }}</text>
            </button>
          </view>
        </template>
        <PetpalEmpty v-else title="还没有宠物档案" description="先补一只宠物，后续所有需求、订单和售后都能直接复用。">
          <button class="petpal-btn petpal-btn--primary" hover-class="none" @click="openPets">去建档</button>
        </PetpalEmpty>
      </PetpalSection>

      <PetpalSection title="正在处理的需求" subtitle="需求页只负责匹配和确认，不和订单页混用。">
        <template v-if="activeRequest">
          <button class="petpal-row-btn" hover-class="none" @click="openRequestDetail(activeRequest.id)">
            ...
            <text class="petpal-row__value">继续</text>
          </button>
        </template>
        <PetpalEmpty v-else title="没有进行中的需求" description="发起新需求后，匹配、确认和下单都会拆到后续页面继续处理。">
          <button class="petpal-btn petpal-btn--secondary" hover-class="none" @click="openCreateRequest(selectedPet?.id)">现在新建</button>
        </PetpalEmpty>
      </PetpalSection>

      <PetpalSection title="接下来处理订单" :subtitle="aftersalesCount ? `另有 ${aftersalesCount} 笔售后单已分流到售后中心` : '订单页只保留履约本身，售后另进售后中心。'">
        <template v-if="upcomingOrders.length">
          <button
            ...
            hover-class="none"
            @click="openOrderDetailPage(item.id, 'overview', 'owner-home')"
          >
            <view class="petpal-row__copy">
      ...
      <PetpalSection title="匹配建议" subtitle="这里只保留你当前最值得看的照料者，不做大卡片堆叠。">
        ...
        <PetpalEmpty v-else title="当前没有匹配建议" description="补齐宠物和时间后，系统会给你更准确的照料者建议。">
      ...
      <view class="petpal-bottom-bar">
        <text class="petpal-note">当前操作对象：{{ selectedPet?.name || '未选择宠物' }}</text>
        <view class="petpal-action-row">
