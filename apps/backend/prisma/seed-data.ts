...
export async function seedDatabase(prisma: PrismaClient) {
  ...
  const managerCaregiverProfile = await prisma.caregiverProfile.create({
    data: withSnowflakeId({
      ...
      serviceRadiusKm: 8,
      serviceCity: '杭州',
      specialtyTags: ['犬类社交', '幼宠适应', '上门喂养'],
      serviceCommitment: '支持每日图文反馈，紧急情况 10 分钟内联系主人。',
      qualificationMaterials: [
        {
          fileId: 'seed-qualification-id-card',
          url: 'https://static.example.test/petpal/caregiver-id-card.jpg',
          name: '身份证明.jpg',
          mimeType: 'image/jpeg',
          size: 245120,
        ...
        {
          fileId: 'seed-qualification-training-cert',
          url: 'https://static.example.test/petpal/caregiver-training-cert.jpg',
          name: '宠物护理培训证书.jpg',
          mimeType: 'image/jpeg',
          size: 312448,
  ...
  const memberPet = await prisma.petProfile.create({
    data: withSnowflakeId({
      ...
      breed: '柯基',
      gender: 'MALE',
      birthday: new Date('2023-05-06T00:00:00.000Z'),
      weightKg: 12.3,
      neutered: true,
      temperamentTags: ['friendly', 'active'],
      feedingNote: '早晚各一次，避免乳制品。',
      allergyNote: '对鸡肉冻干轻微过敏。',
      medicalNote: '2026 年 2 月完成年度体检，肠胃较敏感。',
      emergencyContact: {
        name: '张三',
        phone: '13800009999',
        relation: '主人父亲',
