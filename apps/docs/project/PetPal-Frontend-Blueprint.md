...
- 支付成功后必须先给用户一个明确结果页，告诉他“钱到了没有、下一步做什么”。
- 需求详情必须支持就地排序候选、快速切换照料者和按步骤改时间 / 预算。
- 需求详情必须支持至少一组直接可用的筛选维度，例如预算内、距离近或评分稳定。
- 需求详情与结算页必须展示照料者可信信息快照：经验、提前预约时长、服务范围、专长标签、服务承诺，避免用户只看单价下单。
...
- 第一屏必须直接看到订单状态和主动作。
- 不允许用户先阅读解释文案才能知道去哪。
- 订单详情首屏必须提供状态信号和快速动作，不允许把“支付 / 沟通 / 看履约 / 售后 / 评价”藏在长页深处。
...
- 售后中心首页不讲概念，只讲待处理事项。
- 首屏必须直接锁定“当前最急的一单”，同时展示退款、投诉、沟通三条信号。
- 其他订单必须支持就地展开摘要，先判断再进入详情，不强迫用户整页跳转。
- 每张售后卡片必须回答“为什么要先处理这单”。
- 投诉页必须直接展示处理进度、处理日志和证据入口，不能只保留一个提交表单。
...
- 不把评价表单塞进说明区域。
- 不让用户在长详情页里找评价入口。
- 评价页必须先告诉用户现在是否能评、是否已评，并把评分、标签和展示方式放在首屏完成。
