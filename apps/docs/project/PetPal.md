...
- 审核规则与上传授权
  ...
  - 当前后端上传白名单、孤儿附件清理定时器和相关集成测试已统一改读 `@rbac/api-common` 的共享附件常量，避免资质 / 消息 / 投诉 / 服务记录 / 整改附件在多端与测试中继续漂移；处罚整改 `10` 份附件上限也已从后台页面与后端校验两侧同步收口到共享层。
...
- 共享服务类型标签 helper 已继续落到 `api-common`：
  - `packages/api-common/src/helpers/petpal-display.ts`
    - 新增 `getPetPalServiceTypeLabel(...)`。
    - 共享层现在会统一处理 `BOARDING / WALKING / FEEDING / DOOR_VISIT` 到中文标签的映射，不再让双端平行维护第二份相同表。
- Web / App 已切到共享服务类型标签 helper：
  - `apps/web-frontend/src/pages/frontend/petpal/shared.ts`
    - `getPetPalServiceTypeLabel(...)` 已改为直接复用共享 helper。
    - `petPalServiceTypeOptions` 现在也已改为从共享 helper 生成 label，继续保留 Web 端现有 option 结构。
  - `apps/app-frontend/src/pages/petpal/owner-shared.ts`
    - `serviceTypeLabels` 已改为从共享 helper 生成 label。
    - `serviceTypeOptions` 也已改为复用共享 helper 生成 label，继续保留 App 端现有 description 文案。
- 定向测试与文档同步：
  - `apps/web-frontend/test/petpal-shared.test.ts`
    - 已补服务类型标签 helper 的稳定性断言，锁住共享展示口径。
  - 文档已同步：
    - `apps/docs/project/PetPal.md`
    - `docs/implementation-history.md`
    - `docs/project-memory.md`
...
- 本轮没有改页面结构、流程或状态存储，只继续收口跨端纯展示 helper。
- 已确认 Web / App 两端现在都会复用同一套服务类型中文标签，但两端 option 的描述文案、布局和页面组合逻辑仍保留在各自前端层。
- 已确认本轮没有把 option 描述文案或其它端侧语义误抽到共享层，`api-common` 仍保持纯函数边界。

风险与缓解：

- 风险：服务类型中文标签已经集中到共享层，后续若调整命名，会同时影响 Web / App 的多处筛选项、列表标题和摘要文案。
- 缓解：本轮已补共享 helper 直测，后续如要调整文案，先在共享测试里锁住目标输出再推进。

- 风险：剩余还能继续共享的展示 helper 已经越来越零散，继续细拆可能会让切片收益下降。
- 缓解：本轮之后优先重新评估共享展示层剩余收益，必要时回到消息状态或真实业务页面闭环推进下一块更高价值的开发。

下一步（1-3）：

1. 继续检查 Web / App 共享入口里是否还存在少量高收益、低风险的纯函数重复。
2. 如果展示层剩余收益继续下降，就转回消息状态模块或订单闭环页面，选择下一块更有价值的业务收口点。
3. 继续按最小切片推进 PetPal 收口，每轮只做局部改动、定向验证、本地提交和文档同步。

### 14.266 2026-04-04（P3-M1 Slice 266）

**概述**：上一轮已经把 tag 文本 helper 收口到共享层，但 Web `shared.ts`、App `owner-shared.ts` 和 `rebuild/shared.ts` 里仍各自保留了一些基础展示映射，包括 `ServiceLogType` 的中文标签和订单 tone 判断。本轮继续沿“只抽纯展示 helper、不碰页面交互”的原则，把这两类 helper 提升到 `@rbac/api-common`，让双端只保留各自的 note 文案和页面组合逻辑。

已完成：

- 共享服务记录标签与订单 tone helper 已继续落到 `api-common`：
  - `packages/api-common/src/helpers/petpal-display.ts`
    - 新增 `getPetPalServiceLogTypeLabel(...)`。
    - 新增 `getPetPalOrderTone(...)`。
    - 共享层现在会统一处理服务记录类型中文标签以及订单状态到 `success / warning / danger / neutral` 的 tone 映射。
- Web / App 已切到共享 helper：
  - `apps/web-frontend/src/pages/frontend/petpal/shared.ts`
    - `petPalServiceLogOptions` 已改为复用共享 `ServiceLogType` 标签 helper 生成 label。
    - Web 端继续保留各自的服务记录 note 文案，不把说明文案抽进共享层。
  - `apps/app-frontend/src/pages/petpal/owner-shared.ts`
    - `getServiceLogTypeLabel(...)` 与 `getOrderTone(...)` 已改为复用共享 helper。
  - `apps/app-frontend/src/pages/petpal/rebuild/shared.ts`
    - `serviceLogOptions` 已改为通过共享标签 helper 生成 label。
    - `getOrderToneClass(...)` 已改为复用 `owner-shared.ts` 暴露的共享 tone helper，不再手写第二份状态映射。
- 定向测试与文档同步：
  - `apps/web-frontend/test/petpal-shared.test.ts`
    - 已补服务记录标签和订单 tone helper 的稳定性断言，锁住共享展示口径。
  - 文档已同步：
    - `apps/docs/project/PetPal.md`
    - `docs/implementation-history.md`
    - `docs/project-memory.md`
...
- 本轮没有改页面结构、提交流程或状态存储，只继续收口跨端纯展示 helper。
- 已确认 Web / App 两端现在都会复用同一套服务记录标签和订单 tone 映射，但各自的服务记录 note 文案仍保留在端侧调用点。
- 已确认本轮没有把 UI 样式类名、页面说明文案或其它端侧依赖错误抽进 `api-common`，共享层仍保持纯函数边界。

风险与缓解：

- 风险：订单 tone 映射已经集中到共享层，后续若调整状态到视觉 tone 的对应关系，会同时影响 App 的多处状态着色。
- 缓解：本轮已补共享 helper 直测，后续如要调整映射，先在共享测试里锁住目标行为再推进。

- 风险：剩余还能继续共享的展示 helper 已经越来越碎，继续硬拆可能会让参数复杂度上升而收益下降。
- 缓解：本轮之后优先重新评估共享展示层剩余收益，必要时回到消息状态或业务页面闭环做下一块更高价值的开发。

下一步（1-3）：

1. 继续评估 Web / App 共享入口里是否还存在少量高收益、低风险的纯函数重复。
2. 如果展示层剩余收益继续下降，就回到消息状态模块或订单闭环页面，选择下一块更有价值的业务收口点。
3. 继续按最小切片推进 PetPal 收口，每轮只做局部改动、定向验证、本地提交和文档同步。
...
- 共享 scope/runtime identity helper 已继续落到 `api-common`：
  - `packages/api-common/src/helpers/petpal-message-composer.ts`
    - 新增 `getPetPalMessageComposerScopeWithLegacyAdoption(...)`。
    - 新增 `resolvePetPalMessageComposerRuntimeIdentity(...)`。
    - 共享层现在会统一处理“按 drafts/recoveries 解析当前 scope，并在需要时先认领 legacy shared 记录，再产出运行时 resolved identity”这段纯流程。
- Web / App 消息状态模块已切到共享 scope/runtime identity helper：
  - `apps/web-frontend/src/pages/frontend/petpal/message-composer-state.ts`
    - `getPetPalMessageComposerScope(...)` 现在直接消费共享 scope helper。
    - `persistPetPalMessageDraft(...)` 与 `setPetPalMessageRecovery(...)` 现在直接消费共享 runtime identity helper，本地不再手写 `resolvePetPalMessageComposerIdentity(..., currentScope)`。
  - `apps/app-frontend/src/pages/petpal/message-composer-state.ts`
    - App 端同一批 scope/runtime identity 流程也已切到共享 helper，不再维护第二份近似实现。
- 定向测试与文档同步：
  - `apps/web-frontend/test/petpal-shared.test.ts`
    - 已补 scope/runtime identity helper 直测，锁住 recovery scope 继承和 legacy shared 草稿认领后的 runtime identity 行为。
  - 文档已同步：
    - `apps/docs/project/PetPal.md`
    - `docs/implementation-history.md`
    - `docs/project-memory.md`
...
- 本轮没有改消息发送协议、消息草稿缓存规则或双端存储介质，只继续收口 scope/runtime identity 纯 helper。
- 已确认 Web / App 两端现在都会复用同一套当前 scope 解析和运行时 identity 解析流程，消息状态模块的纯状态解析重复继续下降。
- 已确认状态写回与存储同步仍保留在端侧，没有把副作用错误抽进共享层。

风险与缓解：

- 风险：当前消息状态模块剩余的重复越来越靠近存储 API 和副作用时机，继续强行共享的收益会继续下降。
- 缓解：本轮先收完 scope/runtime identity 纯逻辑；下一轮会优先重新评估是否继续在这条线上推进，还是切到其它更高收益的 PetPal 模块。

- 风险：共享 helper 现在承接了更多 scope 继承与 legacy 认领语义，后续如果要改 scope 优先级，会同时影响 Web / App。
- 缓解：本轮已补共享 helper 直测和现有 Web 消息状态回归测试，后续如要改规则，仍可先在共享测试里锁住目标行为再推进。

下一步（1-3）：

1. 重新评估消息状态模块剩余重复的收益，避免继续在存储 glue 上过度抽象。
2. 继续检查 PetPal 其它状态模块里是否有类似“纯状态解析重复但副作用仍可留端侧”的收口机会。
3. 继续按最小切片推进 PetPal 收口，每轮只做局部改动、定向验证、本地提交和文档同步。
...
- 共享 persisted snapshot 类型与 helper 已继续落到 `api-common`：
  - `packages/api-common/src/types/petpal.ts`
    - 新增共享的 `PersistedPetPalMessageComposerSnapshot`。
  - `packages/api-common/src/helpers/petpal-message-composer.ts`
    - 新增 `PersistedPetPalMessageDraftRecord`、`PersistedPetPalMessageRecoveryRecord`、`PersistedPetPalMessageComposerRecords`。
    - 新增 `createEmptyPersistedPetPalMessageComposerRecords(...)`、`compactPersistedPetPalMessageComposerRecords(...)`、`toPublicPersistedPetPalMessageComposerSnapshot(...)`。
- Web / App 消息状态模块已切到共享 persisted snapshot helper：
  - `apps/web-frontend/src/pages/frontend/petpal/message-composer-state.ts`
    - persisted snapshot 类型别名、空快照工厂、快照裁剪和 public snapshot 转换已切到共享 helper。
    - Web 端仍继续只在本地保留 `fallbackUploadedAt` 兼容、旧附件 `uploadedAt` 回填和 `localStorage` 读写，不把这些端侧差异抽进共享层。
  - `apps/app-frontend/src/pages/petpal/message-composer-state.ts`
    - App 端同一批 persisted snapshot 纯函数也已切到共享 helper，不再维护第二份近似实现。
- 定向测试与文档同步：
  - `apps/web-frontend/test/petpal-shared.test.ts`
    - 已补空快照、快照裁剪和 public snapshot 转换的共享断言。
  - 文档已同步：
    - `apps/docs/project/PetPal.md`
    - `docs/implementation-history.md`
    - `docs/project-memory.md`
...
- 本轮没有改消息发送协议、消息草稿缓存淘汰规则或双端存储介质，只继续收口 persisted snapshot 纯函数。
- 已确认 Web / App 两端现在都会复用同一套 persisted snapshot 类型、空快照工厂、快照裁剪和 public snapshot 转换规则，消息状态模块的纯逻辑重复继续下降。
- 已确认 Web 端历史附件 `uploadedAt` 回填以及 Web / App 各自的存储读写链路仍保留在端侧，没有被错误抽进共享 helper。

风险与缓解：

- 风险：当前共享层还没有接管 persisted snapshot 的完整原始值归一化与存储值解析，双端状态模块里仍有一批近似的 parse scaffolding。
- 缓解：本轮继续只抽 persisted snapshot 纯 helper，下一轮若继续推进，再优先处理完全无副作用的原始值归一化与 parse scaffolding。

- 风险：共享 helper 现在承接了更多消息草稿升级语义，后续调整 persisted snapshot 口径时会同时影响 Web / App。
- 缓解：本轮已补共享层直测和现有消息状态回归测试，后续如要改 persisted snapshot 规则，仍可先在共享测试里锁住期望行为再推进。

下一步（1-3）：

1. 继续评估消息草稿状态里原始值归一化、persisted snapshot parse scaffolding 和存储值解析是否还能再下沉到共享层。
2. 继续检查导出状态、投诉状态或其余 PetPal 页面里是否还有类似的 snapshot helper 重复值得收口。
3. 继续按最小切片推进 PetPal 收口，每轮只做局部改动、定向验证、本地提交和文档同步。
...
- Web 照料者资料页已切到受管资质附件上传：
  - `apps/web-frontend/src/pages/frontend/petpal/PetPalCaregiverProfileView.vue`
    - “材料名称 + 材料链接 + 添加材料”这组手填表单已移除，改成文件选择、受管上传、查看和移除的资质材料列表。
    - 上传时直接复用现有 `uploadAttachmentFile(...)`，并绑定：
      - `tag1 = PETPAL_CAREGIVER_QUALIFICATION_ATTACHMENT_TAG`
      - `tag2 = caregiverProfile.id`
    - 页面已接入共享资质附件限额：
      - 最多保留 `PETPAL_CAREGIVER_QUALIFICATION_MAX_COUNT`
      - 每次最多上传 `PETPAL_CAREGIVER_QUALIFICATION_UPLOAD_MAX_COUNT`
      - 单张大小不超过 `PETPAL_CAREGIVER_QUALIFICATION_ATTACHMENT_MAX_SIZE_BYTES`
    - 如果当前照料者档案还没有生成 `id`，点击上传会先提示“请先保存一次基础资料，再上传资质材料”，避免无归属附件落地。
- 历史手填资质材料仍保持兼容：
  - 旧记录里的 `mimeType = link/manual`、`size = 0` 仍会正常显示。
  - 新页面会把这类旧记录继续按“可查看 / 可移除”的方式承接，不会因为改成受管上传而丢失既有档案内容。
- Web 资料页材料展示已增强：
  - 材料列表现在会显示文件大小和上传日期。
  - “当前没有资质材料”空态文案已改成更贴近受管上传的说明，不再引导用户手填外链。
...
- 已确认本轮只改 Web 入驻资料页的前端交互，没有改后端照料者档案协议、上传白名单或附件治理逻辑。
- 已确认 Web 端新上传的资质材料现在会和 App 端一样纳入受管附件链路，而历史手填链接记录仍可继续查看与删除，迁移过程没有破坏性。
- 已确认保存照料者资料的 payload 仍然沿用现有 `qualificationMaterials` 结构，后端审核预览和后台资料查看链路不需要同步改造。

风险与缓解：

- 风险：Web 资料页当前仍只允许上传图片，PDF 等非图片证书材料暂未开放。
- 缓解：这与 App 端当前受管上传能力保持一致；如果后续需要开放更广的证书格式，再统一扩展双端上传器。

- 风险：旧手填链接记录和新受管上传记录会在同一列表里并存，个别旧数据仍可能显示“大小未知”。
- 缓解：页面已对 `size <= 0` 做兼容展示；后续如果要进一步清理历史手填数据，可以再做一次性迁移或后台治理。

下一步（1-3）：

1. 继续评估后台上传白名单、附件清理定时器和相关集成测试里的 PetPal 附件标签是否也要改读共享常量。
2. 继续评估是否要把 Web / App 的受管附件元数据构造抽成共享 helper，减少前端各页重复拼装 `fileId/url/name/size/uploadedAt`。
...
- Web 端已补旧匿名缓存短期清理：
  - `apps/web-frontend/src/pages/frontend/petpal/message-composer-state.ts`
    - 新增 `MAX_LEGACY_ANONYMOUS_PERSISTED_AGE_MS = 24h`，本地缓存裁剪时会区分匿名旧快照与已归属用户的正式缓存。
    - 仅对 `userId === ''` 的旧匿名记录使用 24 小时清理窗口；已经迁移到 `userId + scope + orderId` identity 的记录仍保持 7 天保留，不会误伤当前用户草稿。
    - 线程容量裁剪仍按 `userId + scope` 分桶执行，本轮没有放宽线程上限，也没有改变当前用户侧的新缓存优先级。
- App 端已补同一套匿名缓存清理规则：
  - `apps/app-frontend/src/pages/petpal/message-composer-state.ts`
    - `uni` 本地存储中的旧匿名消息草稿 / 恢复态也已改为 24 小时优先清理，确保 Web / App 在匿名遗留缓存保留窗口上保持一致。
    - 已认领的用户级缓存仍按原 7 天窗口保留，跨页面返回或重新进入应用时的恢复体验不受影响。
- Web 定向单测已补保留边界覆盖：
  - `apps/web-frontend/test/petpal-message-composer-state.test.ts`
    - 新增“匿名旧快照会早于用户级缓存过期”用例，验证 24 小时匿名清理窗口和 7 天用户窗口同时成立。
    - 已确认同一次裁剪中，匿名旧草稿与匿名旧恢复态都会一起被移除，而同时间戳的用户级记录仍会保留。
...
- 已确认本轮没有改动消息发送接口、附件上传链路、页面 identity 透传和匿名迁移入口，变化继续收敛在 Web / App 两端共享消息状态模块和对应定向单测。
- 已确认匿名旧缓存现在既能在首次命中时迁移，也会在长期无人认领时更早过期，避免“理论兼容、实际长期残留”的升级尾巴继续扩散到共享设备场景。
- 已确认 24 小时短期窗口只作用于 `userId === ''` 的匿名旧记录，不会缩短当前用户已经认领的消息草稿 / 恢复态保留时间。

风险与缓解：

- 风险：如果用户在升级后超过 24 小时才重新进入某条旧匿名线程，该线程的历史草稿可能已经被短期清理掉。
- 缓解：这类记录本身没有用户归属，优先级低于共享设备隐私和残留控制；而且本轮仍保留首次命中即迁移，只缩短“长期无人认领”那部分残留的寿命。

- 风险：当前短期清理仍然发生在消息状态读取 / 裁剪链路，不会在应用启动时主动全量扫描旧匿名缓存。
- 缓解：本轮先保持改动边界最小，用现有裁剪入口收掉最主要的匿名残留问题；若后续仍观察到大量旧 key 长期滞留，再单独评估启动期一次性清理。

下一步（1-3）：

1. 继续评估是否需要把旧匿名缓存的一次性启动期清理做成显式迁移任务，进一步减少共享设备上的遗留键。
2. 继续评估订单消息与投诉证据是否要统一升级为带 `fileId` 的受控附件快照，进一步提升附件引用追踪可靠性。
3. 继续按切片节奏推进局部改动、定向验证、本地提交和文档同步，不回到无边界大改。

### 14.239 2026-04-04（P3-M1 Slice 239）

**概述**：上一轮已经把消息缓存主键升级到“用户 + 角色 + 订单”复合 identity，但旧版本地快照虽然还能被解析，实际页面读写已经不会再命中这些匿名记录。这会让升级前留在浏览器或 App 本地的未发送草稿看起来“格式兼容、实际丢失”。本轮继续沿消息状态主链路补一层显式迁移，在首次读取时把旧匿名快照认领到当前登录用户，补齐真正的升级兼容。

已完成：

- Web 端已补旧匿名缓存迁移：
  - `apps/web-frontend/src/pages/frontend/petpal/message-composer-state.ts`
    - 新增旧匿名记录认领逻辑；当当前用户 identity 下尚无命中项时，会继续尝试匹配同订单的匿名旧记录，并在首次读取时迁移到当前用户。
    - 对于更早只有 `orderId` 的匿名 `shared` 快照，若页面当前已明确 `owner / caregiver` 作用域，迁移时会直接落到当前作用域，避免继续停留在旧的匿名共享桶。
    - 对于 slice 237 遗留的“有 `scope` 但无 `userId`”记录，迁移时会保留原作用域，只补齐当前用户 identity。
    - 迁移完成后会立即回写本地存储并删掉旧匿名 key，不会长期并存两套记录。
- App 端已补同一套匿名缓存迁移：
  - `apps/app-frontend/src/pages/petpal/message-composer-state.ts`
    - `uni` 本地存储中的旧匿名消息草稿 / 恢复态也已支持首次读取即认领到当前登录用户。
    - App 端与 Web 端现在对匿名旧缓存采用同一套迁移规则，不再出现一端能恢复、一端继续丢失的差异。
- Web 定向单测已补旧缓存迁移覆盖：
  - `apps/web-frontend/test/petpal-message-composer-state.test.ts`
    - 新增“匿名 shared 快照迁移到当前用户显式作用域”覆盖。
    - 新增“匿名 scoped 快照迁移后保留原作用域”覆盖。
...
- 已确认本轮没有改动消息发送接口、上传协议、页面入参与消息展示层，变化继续收敛在 Web / App 两端共享消息状态模块和对应定向单测。
- 已确认旧匿名快照现在不仅“能解析”，而且会在首次命中时被迁移到当前登录用户，避免升级后出现看似兼容但实际无法恢复的假兼容状态。
- 已确认匿名旧快照迁移只会在当前 identity 下没有现存命中项时触发，不会覆盖已经存在的用户级新缓存。

风险与缓解：

- 风险：旧匿名快照一旦被某个登录用户首次认领，就会转成该用户的私有缓存；在共享设备场景下，另一个用户将不再看到这份旧草稿。
- 缓解：旧版本本来就没有用户隔离，本轮优先把升级后“无法恢复任何人草稿”的问题收掉；迁移后的行为至少回到明确、可预测的用户隔离。

- 风险：当前迁移触发点仍然是首次读取，不会在应用启动时全量扫描并批量搬迁所有旧匿名记录。
- 缓解：本轮优先保持迁移成本和副作用最小，只在用户真正进入对应订单线程时认领旧记录；若后续确认仍需一次性清库，再单独补显式迁移任务。

下一步（1-3）：

1. 继续评估是否需要对长期未命中的旧匿名缓存补一次启动期清理，避免共享设备上长期残留历史匿名 key。
2. 继续评估订单消息与投诉证据是否要统一升级为带 `fileId` 的受控附件快照，进一步提升附件引用追踪可靠性。
3. 继续按切片节奏推进局部改动、定向验证、本地提交和文档同步，不回到无边界大改。
