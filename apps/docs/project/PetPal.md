...
| 订单与履约 | 接单、签到、服务日志、完成确认、超时处理、时间线 | 基本完成，超时治理未完善 | P2-M1 收口异常态与提醒 |
| 消息与回传 | 订单会话、图文消息、过程媒体、未读数 | 基本完成，缺跨订单聚合与主动通知 | P2-M1 |
...
| App 端 | 约 50% | 已完成 PetPal 化基础页面，但仍未重构为完整的真实业务主应用 |
| 测试与审计 | 约 75% | 后端 PetPal 集成测试较完整，前端仍缺统一收口验证与验收材料 |
| 整体项目 | 约 70% | 已明显超出最小原型，但尚未达到文档定义的完整交付态 |
...
| 履约链路 | 后端/Web 基本完成，App 未完成 | 接单、签到、服务日志、签退、确认完成已落地到后端和 Web，App 尚未接成完整动作流 |
| 支付与退款 | 基本完成 | 支付/退款记录、回调审计、退款进度、导出能力已具备 |
| 评价与投诉 | 后端/Web 基本完成，App 部分完成 | Web 端已支持评价、投诉、售后时间线；App 当前以查看为主，缺少完整提交流程 |
| 管理后台 | 基本完成 | 根级 `/petpal-admin/*` 已承载投诉、照料者审核、回调审计、告警队列；处罚、规则发布、运营看板仍未补齐 |
| 消息与在线沟通 | 基本完成 | 订单详情会话、附件回传、未读态、Web/App 工作台摘要已落地；仍缺跨订单消息中心与主动提醒 |
| 收益与数据分析 | 未完成 | 照料者收入分析、经营指标和平台运营指标仍未完整实现 |
...
2. 跨订单消息中心、主动提醒与统一消息聚合尚未落地。
3. 健康记录、资质材料、收益分析、规则发布、违规处罚、运营看板仍缺完整前后端闭环。
4. 统一验收层尚未收口，前端定向验证、人工验收脚本、论文素材还不够完整。
...
#### 第一优先级：App 端完全 PetPal 化

1. 重构 `pages/petpal/index`，改成“主人工作台 + 照料者工作台 + 售后入口”的一体化页面。
2. 将 App 端订单详情补齐为可执行页面，而不是只展示：
   - 主人确认完成
   - 提交评价
   - 发起投诉
   - 照料者接单 / 签到 / 服务日志 / 签退
3. 清理 App 端残留模板命名和产品外观不一致问题，使移动端真正成为 PetPal 主应用。
