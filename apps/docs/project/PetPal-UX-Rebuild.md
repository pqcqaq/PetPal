...
- Web 前台已开始真实路由拆分：
  - `/petpal` 已切为新的主人服务台，只承接宠物、需求、订单和匹配主流程。
  - `/petpal/caregiver` 已新增独立照料者工作台，承接照料者档案、服务配置和基础履约动作。
  - `/petpal/legacy` 保留为兼容混合工作台，用于承接尚未完全迁出的高级导出、资质上传和高级履约。
...
- Web 前台虽然已完成第一刀“主人 / 照料者 / legacy”路由拆分，但照料者资质上传、服务记录媒体上传、签退与部分高级兼容动作仍留在 `legacy`，拆分尚未收口。

### 10.3 下一步

- 先继续把 Web 前台剩余的高频照料者动作从 `legacy` 迁到独立照料者页，压缩兼容页职责。
