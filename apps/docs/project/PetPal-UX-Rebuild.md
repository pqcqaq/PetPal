...
- App 端订单列表、售后中心与四类结果页已补第一轮 focused return：
...
- App 端订单详情已继续补第三轮来源说明：
  - 从消息中心和支付 / 退款 / 投诉 / 评价结果页进入订单详情时，当前会带 `orderId + tab + source` 的轻量 page-context。
  - `pages/order-detail/index.vue` 首屏会解释当前是从哪类任务或结果页进入，并说明为什么把用户带到沟通 / 售后或概览分栏，而不再只是无说明落页。
- App 端订单详情来源说明已继续扩到高频摘要与队列入口：
  - 主人首页、订单队列、售后中心、照料者首页、履约订单和收益页进入订单详情时，也会带来源上下文。
  - 订单详情首屏现在不只解释消息或结果页回流，也能解释“你刚才是从哪个摘要/队列入口点进来的”。
- App 端通知 / 提醒到订单详情的高频动作已继续补直达分流：
  - 通知中心与提醒中心里的单笔订单任务，当前会优先直达订单详情的沟通 / 履约 / 售后分栏，而不是一律停在消息中心、售后中心或履约队列。
  - `pages/order-detail/index.vue` 也已开始解释“你刚才是从通知中心还是提醒中心点进来的”，减少系统待办回流后的二次判断。
- App 端订单详情已继续补 entry reason：
  - 通知直达和支付 / 退款 / 投诉 / 评价结果页进入订单详情时，当前会额外带上更细的 `reason`。
  - `pages/order-detail/index.vue` 首屏现在不只解释“从哪来”，也会直接解释“现在先做什么”，把动态 notice 从来源说明继续推进到动作说明。
- App 端中间页已开始继续透传 detail reason：
  - 订单页、售后中心和消息中心现在会承接上一跳写入的 `detailReason`。
  - 结果页或通知先把用户带到中间页后，再点进订单详情时，当前仍会保留上一跳的动作原因，不会退化成普通队列入口。
- App 端订单详情页顶部决策区已继续收口为“先处理什么”结构：
  ...
  - 沟通分栏已继续改成“聊天工作区 + 底部发送区”，履约分栏已改成“最新节点 + 服务回传”，售后分栏已改成“售后摘要 + 退款/投诉卡片 + 售后动态”，不再继续堆旧式时间线和说明块。
- App 端订单列表页已继续收口为任务队列：
  - `pages/petpal/orders.vue` 已改成“当前最急订单 + 可展开订单队列”结构，不再把用户先丢进统计卡和同质化列表卡。
  - 列表内现在可以直接展开单笔订单处理沟通、履约或售后，订单页开始更接近手机 App 的任务列表而不是网页目录。
- App 端售后中心已继续收口为任务页：
...
- 主人端订单详情首屏、支付结果页、退款结果页、投诉结果页、售后中心、评价页和投诉页已经开始统一到动作优先结构，结果页到订单 / 售后队列的第一轮焦点回流也已补齐。
...
- 继续补 App 订单详情、消息中心、提醒中心与结果页之间更细的 notice 和主动引导；当前已进入消息/提醒/详情/队列入口 page-context 承接阶段，通知 / 提醒中心里的单笔订单高频动作、结果页回流以及中间页二次进入详情都已开始带动态 `reason`，但更多聚合待办的动态分发仍需继续补齐。
