...
- Web 前台已开始真实路由拆分：
  ...
  - `/petpal/legacy` 保留为兼容混合工作台，用于承接尚未完全迁出的高级导出和少量混合操作。
...
- Web 前台虽然已经完成角色路由拆分，并把照料者高频动作迁回独立页，但主人端高级导出和少量混合兼容动作仍留在 `legacy`，拆分尚未完全收口。

### 10.3 下一步

- 先继续把 Web 前台剩余的主人端高级导出与少量混合兼容动作从 `legacy` 迁到更明确的角色页或后续独立路由。
